...
interface FlattenOptions {
   ...
   * — i.e. we show only today's and future lessons.
   * If "yesterday" context is ever needed — increase it.
   */
  daysInPast?: number;
...
/**
 * Stable unique key of a concrete "occurrence" of a lesson on a day.
 *
 * `dayIndex` is the position of the source `LessonDto` in the
 * `schedules[<day>]` array. That is enough to disambiguate collisions when
 * a group has two different classes at the same time (e.g. a lab split
 * into streams with the same `numSubgroup` and `subject` but different
 * teachers / auditories), and when the API returns two identical entries.
 */
