...
const WEEKS = 6;
/** Порядок дней недели Mon-first → индекс в `Date.getDay()` (Вс=0..Сб=6). */
const MON_FIRST_DOW: readonly number[] = [1, 2, 3, 4, 5, 6, 0];

interface Props {
  /** История статусов по ISO-дням. */
  history: Record<string, FireDayStatus>;
  /** Цвет «горевших» дней — тир текущей серии. */
  accentColor: string;
  now?: Date;
}

/**
 * Календарь активности: сетка последних недель. Каждая клетка — учебный день
 * со статусом (горело / заморозка / пропуск) либо пусто (нет данных / не
 * учебный день). Данные — из `FireCore.history`.
 */
