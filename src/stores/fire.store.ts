    ...
    (set, get) => ({
      ...
      _migrate: async () => {
        ...
        try {
          ...
          // Помечаем миграцию выполненной только при успешном чтении (в т.ч.
          // когда старых данных нет). При исключении флаг остаётся false —
          // иначе временный сбой хранилища навсегда потерял бы старый streak;
          // повторим на следующем запуске.
          set({ migratedFromDiary: true });
        } catch {
          // best-effort: миграция не должна ломать старт приложения; повторим позже
        }
      },
    }),
    {
