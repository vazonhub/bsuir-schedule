...
/** Spotlight padding around the element (pt). */
const SPOTLIGHT_PAD = 6;
/** Gap between the spotlight and the tooltip (pt). */
const TOOLTIP_GAP = Spacing.lg;
/** Rough tooltip height estimate for choosing "above/below" placement. */
const TOOLTIP_ESTIMATE = 180;
/** Text on the accent "Next" button — white in both themes. */
const NEXT_LABEL_COLOR = '#FFFFFF';

const delay = (ms: number) => new Promise<void>((r) => setTimeout(r, ms));

/**
 * Step-by-step tutorial overlay. Dims the screen with four rectangles
 * around the target (no native SVG mask), draws an animated spotlight ring
 * and a tooltip card. Controlled via {@link useTutorial}.
 *
 * Must be mounted so that its absoluteFill matches the window coordinates
 * (target coordinates come from `measureInWindow`).
 */
export const SpotlightOverlay = () => {
  ...
  // Resolved target rect (already padded). null = centered step.
  // During a re-measure we keep the previous rect — the ring smoothly
  // glides from the old target to the new one, without flashing the centered tooltip.
  const [rect, setRect] = useState<TargetRect | null>(null);

  // Spotlight shared values — for a smooth ring/dimming transition.
  ...
  // Resolving the current step's target: measure → (if needed) scroll → re-measure.
  useEffect(() => {
  ...
  // Pass the resolved rect into shared values (first time — instantly).
  useEffect(() => {
    if (!rect) return;
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
      {hasTarget ? (
        // No dimming — only a blinking frame around the target.
        // Tapping the highlighted element also advances.
...
/** Tooltip positioning: below the target if there is room underneath, otherwise above it. */
