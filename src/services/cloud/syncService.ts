...
/**
 * Result of a cloud read. `invalid` (a blob exists but failed
 * validation — garbage or a different schema version) is distinct from
 * `empty`, so the caller doesn't mistake a foreign blob for "cloud is
 * empty" and overwrite it with its own push.
 */
