...
- **Apple Watch app** (watchOS only, iOS 15.1+ / watchOS 10+). Shows the pinned (`defaultGroup`) schedule: today + paging by days/weeks, and can switch to any other group or teacher directly on the watch. Data flow differs from widgets because the watch is a separate device — App Group UserDefaults do **not** sync across devices:
  ...
  - The watch target is created by `plugins/withWatchApp.js` (mirrors `withWidget.js`; hand-rolled pbxproj since `ios/` is gitignored). Swift sources in `targets/watch/` (+ `targets/watch-complication/`) are the source of truth and are copied into `ios/` on every `prebuild`. **Plugin order matters**: `withWatchComplication` is listed _before_ `withWatchApp` in `app.json` because Expo applies xcodeproj mods in reverse, so the watch target exists when the complication wires itself into it. Watch + complication ship inside the main app's EAS archive automatically; one-time provisioning steps are in `RELEASE.md` §0.
