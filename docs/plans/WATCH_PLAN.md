...
| Complications           | **Сделаны** (accessoryInline/circular/rectangular «ближайшая пара», `targets/watch-complication/`)                     |
...
| EAS-сборка/submit часов | Watch+complication **встраиваются в архив основного приложения** автоматически. Провижининг — владелец (RELEASE.md §0) |
...
## 8. Статус пост-MVP фаз

Сделано после MVP:

- **Переключение группы/преподавателя на часах** — пикер (поиск + недавние),
  фетч через API, override поверх снапшота с телефона. `targets/watch/PickerView.swift`.
- **Complications / WidgetKit на watchOS** — «ближайшая пара» в семействах
  inline/circular/rectangular. `targets/watch-complication/`, `plugins/withWatchComplication.js`.
- **EAS**: watch-app и complication встраиваются в архив основного приложения
  автоматически (проверено на Release-сборке) — отдельная EAS-сборка/`eas.json` не нужны.

Осталось (на владельце / вне объёма):

- **Провижининг в Apple Developer**: регистрация `…watchkitapp` и
  `…watchkitapp.complication` + App Group, генерация профилей через `eas credentials`.
  Пошагово — `RELEASE.md` §0.
- **Wear OS** (Android-часы) — вне объёма.
